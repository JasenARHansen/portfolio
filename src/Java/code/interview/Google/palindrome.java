...
public class palindrome {
    ...
    public static Map.Entry<String, String> transform(String value) {
        ...
        if ((value.length() == 0) || (value.length() == 1)) {
            source = value;
            modification = value;
        } else if (Character.toLowerCase(value.charAt(0))
            ...
            source = value.charAt(0) + return_value.getKey() + value.charAt(value.length() - 1);
            modification = value.charAt(0) + return_value.getValue() + value.charAt(value.length() - 1);
        } else {
            // Remove first char
            ...
            if (strip_first.getKey().length() < strip_last.getKey().length()) {
                source = value.charAt(0) + strip_first.getKey() + value.charAt(0);
                modification = value.charAt(0) + strip_first.getValue() + "+";
            } else {
                source =
                        value.charAt(value.length() - 1)
                                + strip_last.getKey()
                                + value.charAt(value.length() - 1);
                modification = "+" + strip_last.getValue() + value.charAt(value.length() - 1);
            }
        }
