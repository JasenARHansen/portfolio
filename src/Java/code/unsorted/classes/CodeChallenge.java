...
public class CodeChallenge {
  ...
  public static void description() {
    ...
    System.out.println(
        "Challenge 9: Given 1 or more array, without merging, find the Nth item in the sequence");
    System.out.println(
  ...
  public static List<Integer> findCommonComplex(int[]... sourceArrays) {
    ...
    for (int index = 0; index < sourceArrays[0].length; index++) {
      ...
      while (((index + 1) < sourceArrays[0].length)
          && (sourceArrays[0][index] == sourceArrays[0][index + 1])) {
        index++;
      }
    }
    return returnValues;
  ...
  private static void addQueen(
      int collumn, List<Tuple<Integer, Integer>> queenList, int[][] board) {
  ...
  private static ArrayList<ArrayList<Integer>> findSubsets(
      int index, int[] numbers, ArrayList<ArrayList<Integer>> returnValue) {
