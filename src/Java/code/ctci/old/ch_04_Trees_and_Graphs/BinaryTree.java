...
public class BinaryTree<T extends Comparable<T>> {
  ...
  private ArrayList<T> buildOrder(BinaryTreeNode<T> node) {
  ...
  public T getRandomNode() {
    ...
    ArrayList<T> order = this.buildOrder(this.root);
  ...
  public String dataBreadth() {
    return dataBreadth(this.root).trim();
  }
  ...
  public String dataPreorder() {
    return dataPreorder(this.root).trim();
  }
  ...
  private Tuple<T, T> validateBST(BinaryTreeNode<T> entryNode) {
    ...
    // Test boundaries
    if (left != null) {
      if (left.rightData().compareTo(entryNode.getData()) <= 0) {
        leftData = left.rightData();
      } else {
        return null;
      }
    }
    if (right != null) {
      if (entryNode.getData().compareTo(right.leftData()) <= 0) {
        rightData = right.leftData();
      } else {
        return null;
      }
    }
