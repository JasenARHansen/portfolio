...
@SuppressWarnings({"DuplicatedCode", "SpellCheckingInspection", "unused", "unchecked"})
public class string_arithmetic {
  ...
  public static List<String> convert(Object value) {
    ...
    if (value == null) {
      ...
      return_list.add("0");
    } else if (value instanceof String) return_list = convertString((String) value);
    else if (value instanceof Integer) return_list = convertNumber((Integer) value);
    ...
    if (return_list != null) trimZeros(return_list);
    return return_list;
  }
    ...
    if (left_list.size() > right_list.size()) {
      ...
      for (int index = left_list.size() - 1; index >= 0; index--) {
        if (Integer.parseInt(left_list.get(index)) < Integer.parseInt(right_list.get(index))) {
          break;
        } else if (Integer.parseInt(left_list.get(index))
            > Integer.parseInt(right_list.get(index))) {
          returnValue = true;
          break;
        }
      }
