package Java.Code.Interview.Riot;
...
public class Riot {
  ...
  static Comparator<Entry<String, Integer>> valueComparator =
      (e1, e2) -> {
  ...
  static Comparator<Entry<String, Integer>> depthComparatorBonus =
      (e1, e2) -> {
  ...
  // Comparator to sort by path depth
  static Comparator<Entry<String, ArrayList<String>>> depthComparator =
      (e1, e2) -> {
        Integer v1 = e1.getKey().length() - e1.getKey().replace(File.separator, "").length();
        Integer v2 = e2.getKey().length() - e2.getKey().replace(File.separator, "").length();
        return v1.compareTo(v2);
