...
TEST(StringCompressionTest, description) {
    testing::internal::CaptureStdout();
    StringCompression::description();
    EXPECT_NE("", testing::internal::GetCapturedStdout());
}

TEST(StringCompressionTest, compress_1) {
...
TEST(StringCompressionTest, compress_2) {
...
TEST(StringCompressionTest, decompress_1) {
...
TEST(StringCompressionTest, decompress_2) {
    auto expected = "string";
    auto source = "string";
    auto result = StringCompression::decompress(source);
    EXPECT_EQ(expected, result);
