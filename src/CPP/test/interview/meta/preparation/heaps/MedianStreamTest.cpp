...
TEST(Interview_MedianStreamTest, description) {
    testing::internal::CaptureStdout();
    MedianStream::description();
    EXPECT_NE("", testing::internal::GetCapturedStdout());
}

TEST(Interview_MedianStreamTest, findMedian_1) {
...
TEST(Interview_MedianStreamTest, findMedian_2) {
...
TEST(Interview_MedianStreamTest, findMinProduct_fail_1) {
    // 1 ≤ arr.length
    int size = 1 - 1;
    vector<int> arr(size);
...
TEST(Interview_MedianStreamTest, findMinProduct_fail_2) {
...
TEST(Interview_MedianStreamTest, findMinProduct_fail_3) {
    // 1 ≤ arr[i]
    int size = 1 - 1;
    vector<int> arr{size};
    EXPECT_EXIT(MedianStream::findMedian(arr), ::testing::ExitedWithCode(3), "");
...
TEST(Interview_MedianStreamTest, findMinProduct_fail_4) {
    // arr[i] ≤ 1,000,000
    int size = (int) pow(10, 6) + 1;
    vector<int> arr{size};
    EXPECT_EXIT(MedianStream::findMedian(arr), ::testing::ExitedWithCode(3), "");
