...
TEST(LeetCode_MinimumPathSumTest, description) {
    testing::internal::CaptureStdout();
    MinimumPathSum::description();
    EXPECT_NE("", testing::internal::GetCapturedStdout());
}

TEST(LeetCode_MinimumPathSumTest, minPathSum_1) {
    auto expected = 7;
...
TEST(LeetCode_MinimumPathSumTest, minPathSum_2) {
    auto expected = 12;
...
TEST(LeetCode_MinimumPathSumTest, minPathSum_fail_1) {
    // 1 ≤ grid.length
    auto size = 1 - 1;
...
TEST(LeetCode_MinimumPathSumTest, minPathSum_fail_2) {
    // grid.length ≤ 200
    auto size = 200 + 1;
    vector<vector<int>> grid(size);
    EXPECT_EXIT(MinimumPathSum::minPathSum(grid), ::testing::ExitedWithCode(3), "");
...
TEST(LeetCode_MinimumPathSumTest, minPathSum_fail_3) {
    // 1 ≤ grid[0].length
    auto size = 1 - 1;
...
TEST(LeetCode_MinimumPathSumTest, minPathSum_fail_4) {
    // grid[0].length< = 200
    auto size = 200 + 1;
    vector<vector<int>> grid{vector<int>(size)};
    EXPECT_EXIT(MinimumPathSum::minPathSum(grid), ::testing::ExitedWithCode(3), "");
...
TEST(LeetCode_MinimumPathSumTest, minPathSum_fail_5) {
...
TEST(LeetCode_MinimumPathSumTest, minPathSum_fail_6) {
    // 0 ≤ grid[i][j]
    auto size = 0 - 1;
    vector<vector<int>> grid{{size}};
    EXPECT_EXIT(MinimumPathSum::minPathSum(grid), ::testing::ExitedWithCode(3), "");
...
TEST(LeetCode_MinimumPathSumTest, minPathSum_fail_7) {
    // grid[i][j] ≤ 200
    auto size = 200 + 1;
    vector<vector<int>> grid{{size}};
    EXPECT_EXIT(MinimumPathSum::minPathSum(grid), ::testing::ExitedWithCode(3), "");
