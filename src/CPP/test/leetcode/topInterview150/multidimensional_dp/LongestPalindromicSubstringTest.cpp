...
TEST(LeetCode_LongestPalindromicSubstringTest, description) {
    testing::internal::CaptureStdout();
    LongestPalindromicSubstring::description();
    EXPECT_NE("", testing::internal::GetCapturedStdout());
}

TEST(LeetCode_LongestPalindromicSubstringTest, longestPalindrome_1) {
    auto expected = "bab";
    auto s = "babad";
...
TEST(LeetCode_LongestPalindromicSubstringTest, longestPalindrome_2) {
    auto expected = "bb";
    auto s = "cbbd";
    auto result = LongestPalindromicSubstring::longestPalindrome(s);
    EXPECT_EQ(expected, result);
...
TEST(LeetCode_LongestPalindromicSubstringTest, longestPalindrome_3) {
...
TEST(LeetCode_LongestPalindromicSubstringTest, longestPalindrome_fail_1) {
    // 1 ≤ s.length
    auto size = 1 - 1;
    string s(size, ' ');
...
TEST(LeetCode_LongestPalindromicSubstringTest, longestPalindrome_fail_2) {
    // s.length ≤ 1000
    auto size = 1000 + 1;
    string s(size, ' ');
    EXPECT_EXIT(LongestPalindromicSubstring::longestPalindrome(s), ::testing::ExitedWithCode(3), "");
