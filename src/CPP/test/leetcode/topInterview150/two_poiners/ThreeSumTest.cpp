...
TEST(LeetCode_ThreeSumTest, description) {
    testing::internal::CaptureStdout();
    ThreeSum::description();
    EXPECT_NE("", testing::internal::GetCapturedStdout());
}

TEST(LeetCode_ThreeSumTest, threeSum_1) {
...
TEST(LeetCode_ThreeSumTest, threeSum_2) {
...
TEST(LeetCode_ThreeSumTest, threeSum_3) {
...
TEST(LeetCode_ThreeSumTest, threeSum_fail_1) {
    // 3 ≤ nums.length
    auto size = 3 - 1;
...
TEST(LeetCode_ThreeSumTest, threeSum_fail_2) {
    // nums.length ≤ 3000
    auto size = 3000 + 1;
    vector<int> numbers(size);
    EXPECT_EXIT(ThreeSum::threeSum(numbers), ::testing::ExitedWithCode(3), "");
