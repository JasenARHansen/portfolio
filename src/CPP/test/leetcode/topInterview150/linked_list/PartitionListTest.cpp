...
TEST(LeetCode_PartitionListTest, description) {
    testing::internal::CaptureStdout();
    PartitionList::description();
    EXPECT_NE("", testing::internal::GetCapturedStdout());
}

TEST(LeetCode_PartitionListTest, partition_1) {
...
TEST(LeetCode_PartitionListTest, partition_2) {
...
TEST(LeetCode_PartitionListTest, partition_3) {
...
TEST(LeetCode_PartitionListTest, partition_4) {
...
TEST(LeetCode_PartitionListTest, partition_5) {
...
TEST(LeetCode_PartitionListTest, partition_fail_1) {
    // n ≤ 200
    auto size = 200 + 1;
...
TEST(LeetCode_PartitionListTest, partition_fail_2) {
    // -100 ≤ Node.val
    auto size = -100 - 1;
    vector<int> values{size};
    EXPECT_EXIT(PartitionList::deserializeList(values), ::testing::ExitedWithCode(3), "");
...
TEST(LeetCode_PartitionListTest, partition_fail_3) {
    // Node.val ≤ 100
    auto size = 100 + 1;
    vector<int> values{size};
    EXPECT_EXIT(PartitionList::deserializeList(values), ::testing::ExitedWithCode(3), "");
...
TEST(LeetCode_PartitionListTest, partition_fail_4) {
    // -200 ≤ x
...
TEST(LeetCode_PartitionListTest, partition_fail_5) {
    // x ≤ 200
    vector<int> headValues{1, 2, 3, 4, 5};
    auto head = PartitionList::deserializeList(headValues);
    auto x = 200 + 1;
