...
TEST(LeetCode_HappyNumberTest, description) {
    testing::internal::CaptureStdout();
    HappyNumber::description();
    EXPECT_NE("", testing::internal::GetCapturedStdout());
}

TEST(LeetCode_HappyNumberTest, isHappy_1) {
    auto expected = true;
    auto n = 19;
...
TEST(LeetCode_HappyNumberTest, isHappy_2) {
    auto expected = false;
    auto n = 2;
    auto result = HappyNumber::isHappy(n);
    EXPECT_EQ(expected, result);
...
TEST(LeetCode_HappyNumberTest, isHappy_fail_1) {
    // 1 ≤ n
    auto n = 1 - 1;
