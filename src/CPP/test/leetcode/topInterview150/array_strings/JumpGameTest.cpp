...
TEST(LeetCode_JumpGameTest, description) {
    testing::internal::CaptureStdout();
    JumpGame::description();
    EXPECT_NE("", testing::internal::GetCapturedStdout());
}

TEST(LeetCode_JumpGameTest, canJump_1) {
    auto expected = true;
...
TEST(LeetCode_JumpGameTest, canJump_2) {
    auto expected = false;
...
TEST(LeetCode_JumpGameTest, canJump_3) {
    auto expected = false;
...
TEST(LeetCode_JumpGameTest, canJump_fail_1) {
    // 1 ≤ nums.length
    auto size = 0;
...
TEST(LeetCode_JumpGameTest, canJump_fail_2) {
    // nums.length ≤ 10^4
...
TEST(LeetCode_JumpGameTest, canJump_fail_3) {
    // 0 ≤ nums[i]
    auto value = -1;
...
TEST(LeetCode_JumpGameTest, canJump_fail_4) {
    // nums[i] ≤ 10^5
