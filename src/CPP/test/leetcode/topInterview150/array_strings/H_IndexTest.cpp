...
TEST(LeetCode_H_IndexTest, description) {
    testing::internal::CaptureStdout();
    H_Index::description();
    EXPECT_NE("", testing::internal::GetCapturedStdout());
}

TEST(LeetCode_H_IndexTest, hIndex_1) {
    auto expected = 3;
...
TEST(LeetCode_H_IndexTest, hIndex_2) {
    auto expected = 1;
...
TEST(LeetCode_H_IndexTest, hIndex_3) {
    auto expected = 1;
...
TEST(LeetCode_H_IndexTest, hIndex_4) {
    auto expected = 1;
...
TEST(LeetCode_H_IndexTest, hIndex_5) {
    auto expected = 2;
...
TEST(LeetCode_H_IndexTest, hIndex_6) {
    auto expected = 3;
...
TEST(LeetCode_H_IndexTest, hIndex_fail_1) {
    // 1 ≤ citations.length
    auto size = 0;
...
TEST(LeetCode_H_IndexTest, hIndex_fail_2) {
...
TEST(LeetCode_H_IndexTest, hIndex_fail_3) {
    // 0 ≤ citations[i]
    auto value = -1;
...
TEST(LeetCode_H_IndexTest, hIndex_fail_4) {
    // citations[i] ≤ 1000
    auto value = 1000 + 1;
    vector<int> citations{1, value, 1};
    EXPECT_EXIT(H_Index::hIndex(citations), ::testing::ExitedWithCode(3), "");
