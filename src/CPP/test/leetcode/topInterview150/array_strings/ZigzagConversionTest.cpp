...
TEST(LeetCode_ZigzagConversionTest, description) {
    testing::internal::CaptureStdout();
    ZigzagConversion::description();
    EXPECT_NE("", testing::internal::GetCapturedStdout());
}

TEST(LeetCode_ZigzagConversionTest, convert_1) {
...
TEST(LeetCode_ZigzagConversionTest, convert_2) {
...
TEST(LeetCode_ZigzagConversionTest, convert_3) {
    auto expected = "A";
    auto s = "A";
    auto numRow = 1;
    auto result = ZigzagConversion::convert(s, numRow);
...
TEST(LeetCode_ZigzagConversionTest, convert_4) {
...
TEST(LeetCode_ZigzagConversionTest, convert_fail_1) {
    // 1 ≤ s.length
    auto size = 1 - 1;
    string s(size, 0);
    auto numRow = 1;
...
TEST(LeetCode_ZigzagConversionTest, convert_fail_2) {
    // s.length ≤ 1000
    auto size = 1000 + 1;
    string s(size, 0);
    auto numRow = 1;
...
TEST(LeetCode_ZigzagConversionTest, convert_fail_3) {
    // 1 ≤ numRows
    auto s = "PAYPALISHIRING";
    auto numRow = 1 - 1;
    EXPECT_EXIT(ZigzagConversion::convert(s, numRow), ::testing::ExitedWithCode(3), "");
...
TEST(LeetCode_ZigzagConversionTest, convert_fail_4) {
    // numRows ≤ 1000
    auto s = "PAYPALISHIRING";
    auto numRow = 1000 + 1;
    EXPECT_EXIT(ZigzagConversion::convert(s, numRow), ::testing::ExitedWithCode(3), "");
