...
#include "../../../../../code/hackerrank/prepare/dataStructures/linkedlist/DetectCycle.cpp"  // NOLINT : Ignore .cpp import

TEST(DetectCycleTest, description) {
    testing::internal::CaptureStdout();
    DetectCycle::description();
    EXPECT_NE("", testing::internal::GetCapturedStdout());
}

TEST(DetectCycleTest, has_cycle_1) {
    auto expected = false;
...
TEST(DetectCycleTest, has_cycle_2) {
    auto expected = false;
    vector<int> source{};
    auto head = DetectCycle::createList(source);
    auto result = DetectCycle::has_cycle(head);
...
TEST(DetectCycleTest, has_cycle_3) {
    auto expected = false;
    vector<int> source{1};
    auto head = DetectCycle::createList(source);
    auto result = DetectCycle::has_cycle(head);
...
TEST(DetectCycleTest, has_cycle_4) {
    auto expected = true;
