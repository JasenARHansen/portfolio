...
void Section2Arrays::arrays() {
    ...
    for (auto i = 0; i < sizeof(first) / sizeof(first[0]); i++) {
        cout << "First index " << i << " has a value of " << first[i] << endl;
    }
    int second[]{3, 0, 1};
    for (auto i = 0; i < sizeof(second) / sizeof(second[0]); i++) {
        cout << "Second index " << i << " has a value of " << second[i] << endl;
    }
    double third[]{1.1, 5.55, 1};
    for (auto i = 0; i < sizeof(third) / sizeof(third[0]); i++) {
    ...
    for (auto i = 0; i < sizeof(fourth) / sizeof(fourth[0]); i++) {
        cout << "Fourth index " << i << " has a value of " << fourth[i] << endl;
    }
    string fifth[3]{"first", "second"};
    for (auto i = 0; i < sizeof(fifth) / sizeof(fifth[0]); i++) {
...
void Section2Arrays::arraysMultiDimensional() {
    ...
    for (auto i = 0; i < sizeof(animals) / sizeof(animals[0]); i++) {
        cout << "Animals Row " << i << ":" << flush;
        for (auto j = 0; j < sizeof(animals[0]) / sizeof(animals[0][0]); j++) {
