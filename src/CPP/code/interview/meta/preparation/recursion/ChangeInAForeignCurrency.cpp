...
class ChangeInAForeignCurrency {
public:
    static void description() {
        cout << R"(Change in a Foreign Currency
    ...
    If you're owed 94 cents in Canada, a shopkeeper will graciously supply you with 95 cents
     instead since there exists a 5-cent coin.
    ...
    Example 2:
        ...
        targetMoney = 75
        output = true
        You can make 75 units with the denominations [17, 29, 29].)" << endl;
    }

    static bool canGetExactChange(int targetMoney, vector<int> &denominations) {
        auto result = false;
        if (targetMoney > 0) {
            for (auto coin: denominations) {
                ...
                result = result || canGetExactChange(targetMoney - coin, denominations);
            }
        }
        return result;
    }
