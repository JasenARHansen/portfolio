...
public:
    ...
    For each index i, you are required to determine the number of contiguous sub-arrays that fulfill
    ...
    Output:
     An array where each index i contains an integer denoting the maximum number of contiguous sub-arrays
     of arr[i]
    Example:
        array = [3, 4, 1, 6, 2]
        output = [1, 3, 1, 5, 1]
        Explanation:
            For index 0 - [3] is the only contiguous subarray that starts (or ends) at index 0 with the
             maximum value in the subarray being 3.
