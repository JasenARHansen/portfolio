...
public:
    static void description() {
        cout << R"(All Wrong
    There's a multiple-choice test with N questions, numbered from 11 to N.
    Each question has 22 answer options, labelled A and B.
    You know that the correct answer for the Ith question is the Ith character in the string C, which is either
    ...
    Your task is to implement the function getWrongAnswers(N, C) which returns a string with N characters,
     the Ith of which is the answer you should give for question I in order to get it wrong (either "A" or "B").
    Constraints:
        1 ≤ N ≤ 100
        Ci∈{'A', 'B'}
    Sample test case #1
