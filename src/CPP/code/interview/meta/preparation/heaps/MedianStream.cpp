...
public:
    ...
    The median of a list of integers is defined as follows.
        If the integers were to be sorted, then:
            If there are an odd number of integers, then the median is equal to the middle integer in
             the sorted order.
    ...
    Example 2:
        ...
        arr = [1, 2]
        output = [1, 1]
        The median of [1] is 1, the median of [1, 2] is (1 + 2) / 2 = 1.5 (which should be rounded
         down to 1).)" << endl;
    }

    static vector<int> findMedian(const vector<int> &arr) {
        assert(!arr.empty());
        ...
        for (auto value: arr) {
            assert(0 < value);
            assert(value < pow(10, 6));
            maxHeap.push(value);
            if (maxHeap.size() - 1 > minHeap.size()) {
                auto change = maxHeap.top();
                maxHeap.pop();
                minHeap.push(change);
            }
            if (maxHeap.size() == minHeap.size()) {
                result.push_back((maxHeap.top() + minHeap.top()) / 2);
            } else {
