...
public:
    ...
    Compute a list of x integers output, the ith of which is the 1-based index in the original array of the
    ...
    Example:
        ...
        The remaining elements are then decremented and added back onto the queue, whose contents are then
         [5, 0, 1, 1, 2].
        ...
        Since they all have an equal value, we remove the one that was popped first, which had the
         initial value of 1 at index 1 in the original array.
        ...
        We remove the one that was popped first, which had the initial value of 2 at index 2 in
         the original array.)" << endl;
    }

    static vector<int> findPositions(const vector<int> &arr, int x) {
        vector<int> result;
        queue<pair<int, int>> values;
        queue<pair<int, int>> process;
        auto index = 1;
        for (auto value: arr) {
            values.emplace(value, index++);
        }
        auto count = x;
        while (!values.empty() && (0 < count)) {
            auto max = 0;
            auto popMax = min(x, (int) values.size());
            // push into queue
            for (index = 0; index < popMax; index++) {
                if (max <= values.front().first) {
                    max = values.front().first;
                }
                process.push(values.front());
                values.pop();
            }
            // remove first largest item in queue and decrement others
            bool removeFirst = true;
            while (!process.empty()) {
                if ((max == process.front().first) && (removeFirst)) {
                    result.push_back(process.front().second);
                    removeFirst = false;
                    process.pop();
                } else {
                    if (0 < process.front().first) {
                        process.front().first--;
                    }
                    values.push(process.front());
                    process.pop();
                }
            }
            count--;
