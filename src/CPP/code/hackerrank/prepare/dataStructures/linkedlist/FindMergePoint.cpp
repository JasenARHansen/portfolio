...
struct SinglyLinkedListNode {
    ...
    explicit SinglyLinkedListNode(int x) : data(x), next(nullptr) {}
};

class FindMergePoint {
public:
    static void description() {
        cout << R"(Find Merge Point of Two Lists:
    ...
    static SinglyLinkedListNode *createList(vector<int> data) {
        SinglyLinkedListNode *result = nullptr;
        if (!data.empty()) {
            result = new SinglyLinkedListNode(data.at(0));
            auto working = result;
            auto index = 1;
            while (index < data.size()) {
                auto *next = new SinglyLinkedListNode(data.at(index));
                working->next = next;
                working = next;
                index++;
            }
