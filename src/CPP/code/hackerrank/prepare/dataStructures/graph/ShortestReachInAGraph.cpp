...
public:
    static void description() {
        cout << R"(Implementation with all offsets in main function.
       Not what I would do.)" << endl;
    }
...
public:
    ...
    Given a graph, determine the distances from the start node to each of its descendants and return the list
     in node number order, ascending.
    If a node is disconnected, it's distance should be -1.
    For example, there are n = 6 nodes in the graph with a starting node s = 1.
    ...
    Explanation:
        We perform the following two queries:
            ...
            (This looks like single direction connections)
            ...
            There is only one edge here, so node 1 is unreachable from node 2 and node 3 has one edge connecting
             it to node 2.
            We then print node 's distance to nodes and 3 (respectively) as a single line of space-separated
             integers: -1 6.
...
public:
    ...
    Explanation:
        We perform the following two queries:
            ...
            There is only one edge here, so node 1 is unreachable from node 2 and node 3 has one edge connecting
             it to node 2.
            We then print node 's distance to nodes and 3 (respectively) as a single line of
