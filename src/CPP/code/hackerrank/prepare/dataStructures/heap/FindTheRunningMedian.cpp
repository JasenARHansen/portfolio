...
public:
    ...
    Returns:
        - float[n]: the median of the array after each insertion, modify the print statement in main to get proper
                     formatting.
    Input Format:
    ...
    static vector<double> runningMedian(const vector<int> &a) {
        ...
        for (auto value: a) {
            if (minHeapForUpper.empty()) {
                minHeapForUpper.push(value);
            } else if (value <= minHeapForUpper.top()) {
                maxHeapForSmaller.push(value);
                ...
                minHeapForUpper.push(value);
                if (minHeapForUpper.size() > maxHeapForSmaller.size() + 1) {
