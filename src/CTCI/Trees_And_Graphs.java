...
public class Trees_And_Graphs {
	...
	public static <T extends Comparable<T>> BinarySearchTreeHeightBalanced<T> convertToBST(T[] array) {
	...
	private static <T extends Comparable<T>> void convertToBST(int start, int stop,
															   BinarySearchTreeHeightBalanced<T> tree, T[] array) {
	...
	public static <X extends Comparable<X>, Y, Z> List<X> findPath(Graph<X, Y, Z> graph, NodeGraph<X, Y, Z> start, NodeGraph<X, Y, Z> stop) {
		return graph.findPathBredthFirstId(start, stop);
	}

	public static List<ArrayList<NodeTree<Integer>>> listOfDepths(BinarySearchTreeHeightBalanced<Integer> tree) {
