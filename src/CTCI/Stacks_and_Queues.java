...
public class Stacks_and_Queues {
    ...
    class MultiStackArray {
        ...
        private List<Integer> stackIndex;
        ...
        private void growStack(int stack) {
            ...
            for (int i = 0; i <= stack; i++) {
                if (this.stackIndex.get(i) >= 0)
                    System.arraycopy(this.stackValues, baseIndex, newStackValues, grownIndex, this.stackIndex.get(i));
            ...
            for (int i = 0; i < this.stackSizes.size(); i++) {
                ...
                if (this.isEmpty(i)) {
                    ...
                    for (int j = 1; j < this.stackIndex.get(i); j++) {
                        stringBuilder.append(", ").append(this.stackValues[baseIndex + this.stackIndex.get(i) - j - 1]);
                    }
                }
    ...
    class StackOfPlates<T extends Comparable<T>> {
        ...
        public T peek() {
            // empty stack case
            if ((this.stackList == null) || (this.stackList.size() == 0) || (this.stackList.get(0).size() == 0)) {
                throw new EmptyStackException();
            }
        ...
        public T popAt(int index) throws Exception {
            ...
            // Input validation
            if ((this.stackList == null) || (this.stackList.size() == 0) || (this.currentStack < index)
            ...
            for (int i = index; i < this.currentStack; i++) {
                this.stackList.get(i).add(0, this.stackList.get(i + 1).remove(this.stackList.get(i + 1).size() - 1));
            }
