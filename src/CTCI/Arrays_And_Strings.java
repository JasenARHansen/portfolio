...
public class Arrays_And_Strings {
    ...
    public static String compressedString(String input) {
        ...
        StringBuilder locaString = new StringBuilder();

        char compairCharacter = input.charAt(0);
        int compairCount = 1;

        for (int i = 1; i < input.length(); i++) {
            if (input.charAt(i) != compairCharacter) {
                locaString.append(compairCharacter);
                locaString.append(compairCount);
                compairCharacter = input.charAt(i);
                compairCount = 1;
            } else {
                compairCount++;
            }
        ...
        return (locaString.length() < input.length()) ? locaString.toString() : input;
    }
    ...
    public static int[][] zeroMatrix(int[][] matrix) {
        Set<Integer> outter = new HashSet<>();
