...
public class MatchingPairs {
  ...
  */
    ...
    public static int matchingPairs(String s, String t) {
        ...
        for (int indexOuter = 0; indexOuter < left.length; indexOuter++) {
            for (int indexInner = indexOuter + 1; indexInner < right.length; indexInner++) {
                swap = right[indexOuter];
                right[indexOuter] = right[indexInner];
                right[indexInner] = swap;
                count = countMatching(left, right);
                if (count > characters) {
                    characters = count;
                }
