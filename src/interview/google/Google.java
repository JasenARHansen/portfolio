...
public class Google {
	...
	private static Double convertNumber(String number) throws Exception {
		...
		return (sign * (convertInteger(base) + (convertInteger(decimal) * Math.pow(10, -decimal.length()))));
	}
	...
	private static boolean[][] floodFill(boolean color, int x, int y, boolean[][] bitMap) {
		...
		if ((y < 0) || (y >= bitMap.length)) {
			System.out.format("Value '%d' is not bin the range of y values [%d, %d)", y, 0, bitMap.length);
			return bitMap;
		}

		if ((x < 0) || (x >= bitMap[y].length)) {
			System.out.format("Value '%d' is not bin the range of x values [%d, %d)", x, 0, bitMap[y].length);
			return bitMap;
		}
	...
	public static void main(String[] args) throws Exception {
		...
		boolean[][] source = {{true, false, true, false}, {false, false, false, true}, {true, false, true, true},
