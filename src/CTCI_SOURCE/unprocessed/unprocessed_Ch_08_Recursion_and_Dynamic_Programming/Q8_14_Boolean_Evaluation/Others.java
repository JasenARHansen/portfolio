package CTCI_SOURCE.unprocessed.unprocessed_Ch_08_Recursion_and_Dynamic_Programming.Q8_14_Boolean_Evaluation;
...
public class Others {
    ...
    public static String insertParensAround(String expression, int ind) {
        ...
        return expression.substring(0, left) + '(' + expression.substring(left, right + 1) + ')' + expression.substring(right + 1);
    ...
    public static int countDPEff(String exp, boolean result, int start, int end, HashMap<String, Integer> cache) {
        ...
        int count = 0;
        if (!cache.containsKey(key)) {
            if ((start == end) && (exp.charAt(start) == '1')) {
                count = 1;
            }
